...
class main
{
	...
	*/
	public function __construct(\phpbb\db\driver\driver_interface $db, \phpbb\controller\helper $helper, \phpbb\request\request_interface $request, \phpbb\template\template $template, \phpbb\user $user, \phpbb\config\config $config)
	{
		$this->db 						= $db;
		$this->helper 					= $helper;
	...
	public function handle($view = '')
	{
		...
		switch ($mode)
		{
			case 'd':
				...
				$month_select_box = $this->make_select_box($month_options, $s_month[0] . '-' . $s_month[1], 's_month', $this->user->lang['SHOW_STATS_FOR_MONTH'], $this->user->lang['GO'], $this->u_action);
				...
				$this->template->assign_vars(array(
					...
					'STATS_MONTH_EXPLAIN'		=> sprintf($this->user->lang['STATS_MONTH_EXPLAIN'], $start_time['month'] . ' ' . $start_time['year']),
			...
			case 'm':
				...
				$year_options = array_merge($year_options, array(
					'all' => $this->user->lang['ALL']
				));
				
				$year_select_box = $this->make_select_box($year_options, ($show_all) ? 'all' : $s_year . ' ', 's_year', $this->user->lang['SHOW_STATS_FOR_YEAR'], $this->user->lang['GO'], $this->u_action);
				...
				$this->template->assign_vars(array(
					...
					'STATS_YEAR_EXPLAIN'	=> sprintf($this->user->lang['STATS_YEAR_EXPLAIN'], $start_time['year'] != $current_time['year'] ? ($show_all ? $start_time['year'] . ' - ' . $current_time['year'] : $start_time['year']) : $current_time['year']),
		...
		$this->template->assign_vars(array(
			'L_TITLE'		=> $this->user->lang['BASIC_STATS'],			
			...
			'AS_ON'			=> sprintf($this->user->lang['AS_ON'], $this->user->format_date($current_time[0])),
		));
		
		$this->template->set_filenames(array('body' => $mode . '_view.html'));
		$page_title = $this->user->lang['STATISTICS'] . ' &bull; ' . $this->user->lang[strtoupper($mode . '_view')];
